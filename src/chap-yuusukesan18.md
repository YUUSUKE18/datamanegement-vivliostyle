...
今回は、その過程で経験したデータ管理<span class="footnote">本記事は、私的利用の範囲にてデータ化しました。自身でデータ化する分には問題となることは少ないですが、本書でお話しする「自炊代行」は、違法であるという判決[1]があります。データ化されるときには、必ず法律を守って行いましょう。</span>について、まとめてご紹介します。
...
ダウンロードURLも、有効期限が設けられており、筆者が利用した時には、30日後に削除されるようになっていました。


## データのパックアップ対応策

電子化されたデータは、PDFで管理することがほとんどでしょう。
...
「自炊代行」は違法という判例は、自炊代行業者に対するものなので、「依頼者」が裁かれる可能性は高くないかもしれません。しかし、印刷された著作物の取り扱いには、十分にご注意ください。
